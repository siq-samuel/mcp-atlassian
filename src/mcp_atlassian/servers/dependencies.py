...
async def get_jira_fetcher(ctx: Context) -> JiraFetcher:
    ...
    try:
        ...
        if jira_token:
            logger.debug("get_jira_fetcher: Using Jira-specific token")
            ...
            user_jira_fetcher = JiraFetcher(config=user_jira_config)
...
async def get_confluence_fetcher(ctx: Context) -> ConfluenceFetcher:
    ...
    try:
        ...
        user_confluence_username = getattr(
            request.state, "user_confluence_username", None
        )

        if confluence_token:
            logger.debug("get_confluence_fetcher: Using Confluence-specific token")
            ...
            user_confluence_fetcher = ConfluenceFetcher(config=user_confluence_config)
